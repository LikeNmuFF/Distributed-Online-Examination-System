      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8 sm:py-12 relative z-10">
        ...
        {showArchitecture && (
          <div className="card-elevated p-8 mb-10 animate-scaleIn">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mt-8 pt-8 border-t border-border-subtle">
              {[
                { title: 'Parallelism', desc: 'Worker threads grade all questions simultaneously' },
                { title: 'Distribution', desc: '3 independent nodes sharing Redis state' },
                { title: 'Load Balancing', desc: 'Least-conn evenly distributes traffic' },
                { title: 'Fault Tolerance', desc: 'Sessions persist across node failures' },
                { title: 'Message Queue', desc: 'Bull queue prevents grading data loss' },
                { title: 'Real-time Sync', desc: 'Socket.IO syncs timers across all nodes' },
              ].map((concept, i) => (
                <div key={i} className="bg-bg-tertiary/30 border border-border-subtle rounded-lg p-4">
                  <p className="text-xs font-display font-bold text-accent mb-1 tracking-widest uppercase">{concept.title}</p>
                  <p className="text-xs text-text-tertiary leading-relaxed">{concept.desc}</p>
                </div>
              ))}
            </div>
          </div>
